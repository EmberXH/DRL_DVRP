...
def main(args):
    ...
    if args.normalized_data:
        ...
        unnorm_b_time *= 480
        unnorm_data = SDVRPTW_Dataset(data.veh_count, 200, 1, unnorm, data.cust_mask, data.b_veh_idx, unnorm_b_time)
    else:
    ...
    with tqdm(loader, "Evaluating") as progress:
        ...
        for b, batch in enumerate(progress):
            env = SDVRPTW_Environment(data, batch[0], None, batch[1], batch[2], pending_cost=0, late_p=0)
            env.is_ortools = True
