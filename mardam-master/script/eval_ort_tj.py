...
def _solve_loop(nodes, veh_count, veh_capa, veh_speed, pending_cost, late_cost, tj_time, distance_with_tj):
    # tjs = []
    # for i in range(tj_time.size(0)):
    #     for j in range(tj_time.size(1)):
    #         tjs_j = []
    #         if tj_time[i, j] != 0.0:
    #             tjs_j.append(i)
    #             tjs_j.append(j)
    #             tjs_j.append(tj_time[i, j])
    #             tjs.append(tjs_j)
    ...
    while tjs:
        ...
        for t, veh_i, to_n in traj:
            ...
            while tjs and t >= tjs[-1][2]:
                tj = tjs.pop()
                dist[tj[0], tj[1]] = distance_with_tj[tj[0], tj[1]]
            break
        else:
            tj = tjs.pop()
            t = tj[2]
...
def main(args):
    ...
    for deg_of_dyna in deg_of_dynas:
        data_path = "./data/tj_n{}m{}_{}_{}_10000/norm_data.pyth".format(problem_size, problem_size // 5, deg_of_dyna,
                                                                 appear_early_ratio)
        data = torch.load(data_path)
        assert isinstance(data, DVRPTW_TJ_Dataset)
        if args.normalized_data:
            ...
            unnorm_dwt *= 100
            unnorm_data = DVRPTW_TJ_Dataset(data.veh_count, 200, 1, unnorm, data.cust_mask, unnorm_tj_time, unnorm_dwt)
        else:
            nodes = data.nodes.clone()
            unnorm_data = DVRPTW_TJ_Dataset(data.veh_count, data.veh_capa, data.veh_speed, nodes, data.cust_mask,
                                            data.changed_locs)
            data.normalize()
        ...
        with tqdm(loader, "Evaluating") as progress:
            ...
            for b, batch in enumerate(progress):
                env = DVRPTW_TJ_Environment(data, batch[0], None, batch[1], batch[2], pending_cost=0, late_p=0)
