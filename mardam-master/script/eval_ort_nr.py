...
def parse_args():
    ...
    parser.add_argument("--is-vrplib", action="store_true", default=False)
...
def main(args):
    ...
    if not args.is_vrplib:
        for deg_of_dyna in deg_of_dynas:
            data_path = "./data/nr_n{}m{}_{}_{}_10000/norm_data.pyth".format(problem_size, problem_size // 5,
                                                                             deg_of_dyna,
                                                                             appear_early_ratio)
            data = torch.load(data_path)
            assert isinstance(data, DVRPTW_NR_Dataset)
            ...
            if args.normalized_data:
                ...
                unnorm[:, :, DEM] *= 200
                unnorm[:, :, TIMES] *= 480
                unnorm_data = DVRPTW_NR_Dataset(data.veh_count, 200, 1, unnorm, data.cust_mask)
            else:
                nodes = data.nodes.clone()
                unnorm_data = DVRPTW_NR_Dataset(data.veh_count, data.veh_capa, data.veh_speed, nodes, data.cust_mask)
                data.normalize()
            ...
            with tqdm(loader, "Evaluating") as progress:
                ...
                for b, batch in enumerate(progress):
                    env = DVRPTW_NR_Environment(data, batch, pending_cost=0, late_p=0)
        ...
        args.normalized_data = False
        data_path = "./data/B-VRP/B-n35-k5.vrp.vrp"
        data = DVRPTW_NR_Dataset.read_instance(data_path)
        if args.normalized_data:
            unnorm = data.nodes.clone()
            unnorm[:, :, LOC] *= 100
            unnorm[:, :, DEM] *= 200
