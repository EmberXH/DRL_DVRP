...
def _solve_loop(nodes, veh_count, veh_capa, veh_speed, b_veh_idx, b_time, pending_cost, late_cost):
    ...
    for _ in range(len(b_time)):
        routes, traj = _solve_cp(nodes, dist, veh_count, veh_capa, veh_speed, b_veh_idx[-1], pending_cost,
                                 late_cost, partial)
...
def ort_solve_dyna(data, pending_cost=200, late_cost=1, no_mp=False):
    if no_mp:
        ...
        for nodes in tqdm(data.nodes):
            routes.append(_solve_loop(nodes, data.veh_count, data.veh_capa, data.veh_speed, data.b_veh_idx, data.b_time,
        ...
        with Pool() as p:
            with tqdm(desc="Calling ORTools", total=data.batch_size) as pbar:
                results = [p.apply_async(_solve_loop, (
                    nodes, data.veh_count, data.veh_capa, data.veh_speed, data.b_veh_idx, data.b_time, pending_cost,
                    late_cost), callback=lambda _: pbar.update()) for nodes in data.nodes]
...
def main(args):
    ...
    if args.normalized_data:
        ...
        unnorm[:, :, DEM] *= 200
        unnorm[:, :, TIMES] *= 480
        unnorm_data = SDVRPTW_Dataset(data.veh_count, 200, 1, unnorm, data.cust_mask, data.b_veh_idx,
                                      [i * 480 for i in data.b_time])
    else:
